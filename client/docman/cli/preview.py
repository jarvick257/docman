def preview(subparser):
    ...
    parser.add_argument("--scans", action="store_true", help="preview scans")
...
def _run(doc, args):
    ...
    auto = not (args.pdf or args.scans)
    if (args.pdf or auto) and doc.pdf is not None:
        print("Previewing pdf")
        cmd = doc.config["INTEGRATION"]["pdf_preview"]
        os.system(f"{cmd} {doc.pdf}")
    if (args.scans or (auto and not doc.pdf)) and doc.scans != []:
